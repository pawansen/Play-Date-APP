...
exports.register = [
 	// Validate fields.
    body("inviteCode").trim().custom((value)=>{
		return UserModel.User.findOne({inviteCode:value},{_id:1}).then((userToken)=>{
			if(value){
				if(userToken){
					body.inviteUserId=userToken._id;
				}else{
    ...
    body("userType").isLength({min:1}).trim().withMessage("UserType must be specified Person,Business."),
    body("deviceID").isLength({min:1}).trim().withMessage("Device ID must be specified."),
	body("deviceToken").isLength({min:1}).trim().withMessage("Device Token must be specified."),
	(req,res) =>{
		try{
						...
						UserModel.User.create(users,function(err){
							if(err){
								...
								UserModel.User.findOne({email:req.body.email,phoneNo:req.body.phoneNo}).then(userResponse =>{
									...
									userData.profileVideoPath = (userResponse.profileVideoPath) ? constants.baseUrl+userResponse.profileVideoPath : null;
 						...
 						if(user != ""){
 							bcrypt.compare(req.body.password,user[0].password,function(err,match){
 								if(match){
									...
									const secret = process.env.JWT_SECRET;
									userData.token = jwt.sign(jwtPayload, secret, jwtData);
									userData.gender = user[0].gender;
									userData.birthDate = user[0].birthDate;
									userData.age = user[0].age;
									userData.profilePic = user[0].profilePic;
									userData.profilePicPath = (user[0].profilePicPath) ? user[0].profilePicPath : null;
									userData.profileVideo = user[0].profileVideo;
									userData.businessImage = (user[0].businessImage) ? constants.baseUrl+"/uploads/userProfilePic/"+user[0].businessImage : null;
									userData.profileVideoPath = (user[0].profileVideoPath) ? user[0].profileVideoPath : null;
									userData.profileVideoThumb = (user[0].profileVideoThumb) ? constants.baseUrl+user[0].profileVideoThumb : null;
									userData.relationship = user[0].relationship;
									userData.personalBio = user[0].personalBio;
									userData.interested = user[0].interested;
									userData.restaurants = user[0].restaurants;
									userData.userType = user[0].userType;
									userData.interestedIn = user[0].interestedIn;
									userData.inviteCode = user[0].inviteCode;
									userData.paymentMode = user[0].paymentMode;
									userData.sourceType = user[0].sourceType;
									userData.sourceSocialId = user[0].sourceSocialId;
									userData.inviteLink = "Hey, I'm on PlayDate . Join me! Download it here: "+constants.baseUrl+"/"+user[0].inviteCode+" and use promo code "+user[0].inviteCode+" and earn "+constants.referralPoints+" PlayDate Coins.";
									let userSession ={
										userID:user[0]._id,
										sessionKey:utility.randomValueHex(32),
										deviceType:req.body.deviceType,
					...
					if(user){
						bcrypt.compare(req.body.password,user.password,function(err,match){
							if(match){
								...
								// }else{
								// 	return apiResponse.unauthorizedResponse(res,"You have not activated your account yet, please verify your account first.");
								// }
							}else{
								return apiResponse.unauthorizedResponse(res,"Invalid login credentials.");
							}
...
exports.UpdateUserProfile = [
 		...
 		body("username").trim().custom((value) => {
			return UserModel.User.findOne({username : value}).then((user) => {
				if (user) {
 			...
 			try{
 				...
 				if(!errors.isEmpty()){
 					...
 					if(req.body.interestedIn){
 						userData.interestedIn = req.body.interestedIn;
 						
 					}		
 					if(req.body.relationship){
 						if(req.body.relationship == "Single" || req.body.relationship == "Taken"){
 							userData.relationship = req.body.relationship;
 						}else{
 							return apiResponse.unauthorizedResponse(res,"Relationship must be specified Single, Taken");
 						}
 					}	
 					...
 					if(req.body.interested){
 						userData.interested = req.body.interested.split(",");
 					}	
 					if(req.body.restaurants){
 						userData.restaurants = req.body.restaurants.split(",");
 					}
 					if(userData){
